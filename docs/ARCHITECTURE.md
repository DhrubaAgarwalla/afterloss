...
2. `mode=explain` → Nova 2 Lite (ap-south-1) with the rule's label, quote and route as context → an answer in EN/HI. If the model fails, the rule's own label is shown instead.
